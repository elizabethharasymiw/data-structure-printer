...
struct ExecutionPlan {
    ...
    std::vector<FDuration> run(const IConfig &cfg, Environment env) const {
        ...
        Detail::run_for_at_least<Clock>(
            ...
            warmup_iterations, Detail::repeat([]() { return Clock::now(); }));
...
#pragma clang diagnostic pop
    ...
    env =
        Catch::Detail::make_unique<Environment>(Environment{resolution, cost});
    return *env;
}
} // namespace Detail
} // namespace Benchmark
...
class BinaryExpr : public ITransientExpression {
  ...
  public:
    constexpr BinaryExpr(bool comparisonResult, LhsT lhs, StringRef op,
                         RhsT rhs)
...
struct Decomposer {
    template <
        typename T,
        std::enable_if_t<!capture_by_value<std::remove_reference_t<T>>::value,
                         int> = 0>
...
constexpr ExtendedMultResult<std::uint64_t> extendedMult(std::uint64_t lhs,
                                                         std::uint64_t rhs) {
